...
function load2() {
	if (window.File && window.FileList && window.FileReader) {
		...
		btnClick.addEventListener("click",function(event) {
							...
							document.getElementById("generics2").innerHTML = "Decrypting...Please wait";
...
function valid4(title1) {
	...
	AESDecryptCtr3(title1, pass, bit);
}
...
function upload2(image) {
	...
	var blob = b64toBlob2(b64Data, 'image');

	saveAs(blob, "decrypted image." + fileExtension);

}
function b64toBlob2(b64Data1, contentType, sliceSize) {
...
function upload4(image) {
	
	var b64Data = image.replace(/^data:image\/\w+;base64,/, '');
	
	var blob = b64toBlob4(b64Data, 'image');

	saveAs(blob, "decrypted image." + fileExtension);

}
function b64toBlob4(b64Data1, contentType, sliceSize) {
	contentType = contentType || '';
	sliceSize = sliceSize || 512;
...
function AESDecryptCtr3(image, password, nBits) {

	var b64Data = image.replace(/^data:image\/\w+;base64,/, '');
	var ciphertext = decodeURIComponent(escape(window.atob(b64Data)));

	console.log('cipher text(image dec)');
