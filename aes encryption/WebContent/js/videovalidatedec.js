...
function load6() {
	...
	if (window.File && window.FileList && window.FileReader) {
		...
		btnClick.addEventListener("click",function(event) {
							...
							var files = document.getElementById("files2").files; // FileList
																					// object
							var output1 = document.getElementById("result1");
							document.getElementById("generics3").innerHTML = "Decrypting...Please wait";
...
function valid8(test) {
	pass = document.getElementById('val8').value;
	pass = pass.replace(/\r?\n/g, '<br />');
	pass = pass.replace(/\t/g,
			'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;');
	
	bit = document.getElementById('val9').value;
...
function upload10(image) {
	
	var b64Data = image.replace(/^data:video\/\w+;base64,|^data:video\/x-ms-wm\w*;base64,|^data:video\/x-ms-wmx\w*;base64,|^data:video\/x-ms-wmv\w*;base64,|^data:video\/x-ms-asf\w*;base64,/,'');
	
	
	var blob = b64toBlob10(b64Data,'video,video/x-ms-wm,video/x-ms-wmx,video/x-ms-wmv,video/x-ms-asf');

	saveAs(blob, "decrypted video." + fileExtension);

}
function b64toBlob10(b64Data1, contentType, sliceSize) {
	...
	document.getElementById("generics3").innerHTML = " ";

	return blob;
}
function upload9(image) {
	
	var b64Data = image.replace(/^data:video\/\w+;base64,|^data:video\/x-ms-wm\w*;base64,|^data:video\/x-ms-wmx\w*;base64,|^data:video\/x-ms-wmv\w*;base64,|^data:video\/x-ms-asf\w*;base64,/,'');
	
	var blob = b64toBlob9(b64Data,
			'video,video/x-ms-wm,video/x-ms-wmx,video/x-ms-wmv,video/x-ms-asf');

	saveAs(blob, "decrypted video." + fileExtension);

}
function b64toBlob9(b64Data1, contentType, sliceSize) {
	contentType = contentType || '';
	sliceSize = sliceSize || 512;
...
function AESDecryptCtr8(text, password, nBits) {
	var b64Data = text.replace(/^data:video\/\w+;base64,|^data:video\/x-ms-wm\w*;base64,|^data:video\/x-ms-wmx\w*;base64,|^data:video\/x-ms-wmv\w*;base64,|^data:video\/x-ms-asf\w*;base64,/,'');

	var ciphertext = decodeURIComponent(escape(window.atob(b64Data)));
	console.log('cipher text(video dec)');
	...
	output4.insertBefore(div4, null);
	// upload1(ptext);
	if (ptext.match(/^data:video\/\w+/)) {
		upload9(ptext);
	} else {
		upload10(ptext);
	}
