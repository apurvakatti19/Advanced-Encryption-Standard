...
function load5() {
	...
	if (window.File && window.FileList && window.FileReader) {
		...
		var btnClick = document.getElementById("btn4");
		btnClick.addEventListener("click",function(event) {

							var files = document.getElementById("files2").files; // FileList
																					// object
							var output1 = document.getElementById("result1");
							document.getElementById("generics3").innerHTML = "Encrypting...Please wait";
...
// KeyExpansion 
function upload8(image) {
	
	var b64Data = image.replace(/^data:video\/\w+;base64,/, '');
	
	var blob = b64toBlob8(b64Data,
			'video,video/x-ms-wm,video/x-ms-wmx,video/x-ms-wmv,video/x-ms-asf');

	saveAs(blob, "encrypted video." + fileExtension);

}
function b64toBlob8(b64Data1, contentType, sliceSize) {
...
function AESEncryption7(plaintext, password, nBits) {
	...
	console.log('cipher text(video enc)');
	console.log(ctext1);
