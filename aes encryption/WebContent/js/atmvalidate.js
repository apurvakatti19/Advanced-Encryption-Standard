...
function valid1() {
	...
	title = document.getElementById('atmnum').value;
	pass = document.getElementById('pwd').value;
	bit = document.getElementById('bits').value;
	...
	// multiply the digits in the even position by two
	for (i = 2; i <= title.length; i++) {
		if (i % 2 == 0) {
			arr_reverse[i] = arr_reverse[i] * 2;
		}
	}
	// if the number in any position exceeds nine,subtract by nine
	for (i = 2; i <= title.length; i++) {
		if (arr_reverse[i] > 9) {
	...
	// add the sum by last digit
	sum1 = sum + parseInt(last);
	// if the result is satisfying the below condition,it is valid
	if (sum1 % 10 == 0) {
		valid = 1;
	}
	...
	if (valid == 1) {
		AESEncryption11(title, pass, bit);
...
function AESEncryption11(plaintext, password, nBits) {
	...
	var ctext1 = ctrTxt + '-' + ciphertext.join('-');

	document.getElementById('encrypt').innerHTML = ctext1;

	AESDecryptCtr11(ctext1, password, nBits);

}
function AESDecryptCtr11(ciphertext, password, nBits) {
	if (!(nBits == 128 || nBits == 192 || nBits == 256))
		return ''; // standard allows 128/192/256 bit keys
	...
	document.getElementById('decrypt').innerHTML = ptext;
